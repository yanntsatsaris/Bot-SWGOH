...
- Report a `database/db.py` function without checking whether the name appears more than
  once. That file used to hold eight shadowed duplicates, and the first match is the one
  that used to be dead. Always run:
...
`docs/` research and plans, worth reading before designing anything new.
