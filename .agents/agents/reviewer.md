---
name: reviewer
description: Deep review for the few changes that genuinely need a stronger model — db.py restructuring, the Postgres translation layer, decomposing scouting.py. Invoke deliberately; this spends scarce premium quota.
# Set this to Claude Sonnet from the model list before first use.
...
1. **Restructuring `database/db.py`.** 1,588 lines, 52 top-level definitions, only a
   round-trip floor. It recently carried eight silently shadowed duplicate definitions.
...
3. **Decomposing `services/scouting.py`.** 2,026 lines; `generate_attack_plan` is 450,
   `_predict_zones` 304, `get_scout_data` 279. Extraction here must preserve behaviour
   with no test net beneath it.
...
- Say what you verified and what you only read. Do not imply you ran something you did not.

## Verification

Every change you approve must come with a way to prove it:
