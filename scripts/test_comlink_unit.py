...
async def fetch_unit_from_comlink(target_base_id: str = "GLREY"):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        async with session.post(
            ...
            cat_list = target_unit.get("categoryIdList", [])
            force_align = target_unit.get("forceAlignment", 0)
            align_str = ALIGNMENT_MAP.get(force_align, f"Inconnu ({force_align})")

            factions = [c.replace("affiliation_", "").replace("profession_", "") for c in cat_list if "affiliation_" in c or "profession_" in c]
            roles = [c.replace("role_", "") for c in cat_list if "role_" in c]
            special = [c for c in cat_list if c in ["galactic_legend", "leader", "crewmember", "capital"]]
            ...
            print("\n📋 Liste brute des catégories (categoryIdList) :")
